...
def index():
    ...
    # lookup(symbol) every share symbol the user has and pull the price from it,
    # and display that as the price, because the price and total have to be dynamic
...
def buy():
    ...
    if request.method == "POST":
        ...
        if not quote:
            ...
            return apology("Must Provide A Whole Number Of Shares")
            ...
            db.execute("INSERT INTO transactions (user_id, symbol, shares, price, date) VALUES (?, ?, ?, ?, ?)", user_id, quote["symbol"], shares, quote["price"], date)
...
def register():
    ...
    try:
        ...
        return apology("Usermame already exists")
